...
import (
	...
	"github.com/gohandson/toybox-ja/solution/section11/step04/parawalk"
...
func ConvertAll(ctx context.Context, root string, to, from Format) error {
	...
	walkfunc := func(ctx context.Context, path string, info fs.FileInfo, err error) (rerr error) {
		...
		// 変換前のファイルを開く
		trace.Log(ctx, "convert", "変換前のファイルを開く")
		...
		// 変換後に画像を保存するファイルを作成
		trace.Log(ctx, "convert", "保存するファイルを作成")
	...
	return parawalk.Walk(ctx, root, walkfunc)
}
